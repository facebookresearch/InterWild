...
class Model(nn.Module):
    def __init__(self, body_backbone, body_box_net, hand_roi_net, rhand_position_net, rhand_rotation_net, lhand_position_net, lhand_rotation_net, hand_trans_net):
        super(Model, self).__init__()
        self.body_backbone = body_backbone
        self.body_box_net = body_box_net

        self.hand_roi_net = hand_roi_net
        self.rhand_position_net = rhand_position_net
        self.rhand_rotation_net = rhand_rotation_net
        self.lhand_position_net = lhand_position_net
        self.lhand_rotation_net = lhand_rotation_net

        self.hand_trans_net = hand_trans_net
        self.joint_regressor = nn.Linear(mano.vertex_num*3, mano.sh_joint_num*3)
        ...
        self.trainable_modules = [self.body_backbone, self.body_box_net, self.hand_roi_net, self.rhand_position_net, self.rhand_rotation_net, self.lhand_position_net, self.lhand_rotation_net, self.hand_trans_net, self.joint_regressor]
    
    def forward_rotation_net(self, hand_feat, hand_coord, rotation_net):
        root_pose_6d, pose_param_6d, shape_param, cam_param = rotation_net(hand_feat, hand_coord)
    ...
    def get_coord(self, root_pose, hand_pose, shape, cam_trans, hand_type):
        batch_size = root_pose.shape[0]
        if hand_type == 'right':
            output = self.mano_layer_right(betas=shape, hand_pose=hand_pose, global_orient=root_pose, transl=cam_trans)
        else:
            output = self.mano_layer_left(betas=shape, hand_pose=hand_pose, global_orient=root_pose, transl=cam_trans)
    ...
    def forward(self, inputs, targets, meta_info, mode):
        ...
        hand_feat = self.hand_roi_net(inputs['img'], lhand_bbox, rhand_bbox) # (2N, ...). left hand + right hand
        lhand_num = len(lhand_bbox); rhand_num = len(rhand_bbox);
        lhand_feat = hand_feat[:lhand_num]
        rhand_feat = hand_feat[lhand_num:]
       
        # hand network
        ljoint_img = self.lhand_position_net(lhand_feat)
        lroot_pose, lhand_pose, lshape, lcam_trans = self.forward_rotation_net(lhand_feat, ljoint_img.detach(), self.lhand_rotation_net)

        rjoint_img = self.rhand_position_net(rhand_feat)
        rroot_pose, rhand_pose, rshape, rcam_trans = self.forward_rotation_net(rhand_feat, rjoint_img.detach(), self.rhand_rotation_net)

        # get outputs
        ljoint_proj, ljoint_cam, lmesh_cam, lroot_cam = self.get_coord(lroot_pose, lhand_pose, lshape, lcam_trans, 'left')
        rjoint_proj, rjoint_cam, rmesh_cam, rroot_cam = self.get_coord(rroot_pose, rhand_pose, rshape, rcam_trans, 'right')
        
        # relative translation
        rel_trans = self.hand_trans_net(rjoint_img.detach(), ljoint_img.detach(), rhand_bbox.detach(), lhand_bbox.detach())

        # get joint coordinates from learnable joint regressor
        lmesh_cam = torch.cat((-lmesh_cam[:,:,0:1], lmesh_cam[:,:,1:]),2) # flip to the right hand mesh
        mesh_cam = torch.cat((lmesh_cam, rmesh_cam))
        joint_cam_learn = self.joint_regressor(mesh_cam.detach().view(-1,mano.vertex_num*3)).view(-1,mano.sh_joint_num,3) # detach mesh to prevent gradients from back propagating
        ljoint_cam_learn = joint_cam_learn[:lhand_num,:,:]
        ljoint_cam_learn = torch.cat((-ljoint_cam_learn[:,:,0:1], ljoint_cam_learn[:,:,1:]),2) # flip back to the left hand
        rjoint_cam_learn = joint_cam_learn[lhand_num:,:,:]
        lmesh_cam = torch.cat((-lmesh_cam[:,:,0:1], lmesh_cam[:,:,1:]),2) # flip back to the left hand
        ...
        joint_cam_learn = torch.cat((rjoint_cam_learn, ljoint_cam_learn),1)
        ...
        if mode == 'train':
            ...
            loss['joint_cam_learn'] = self.coord_loss(joint_cam_learn, targets['joint_cam'], meta_info['joint_valid'] * meta_info['is_3D'][:,None,None]) * 10
...
def get_model(mode):
    ...
    rhand_position_net = PositionNet()
    rhand_rotation_net = RotationNet()
    lhand_position_net = PositionNet()
    lhand_rotation_net = RotationNet()
    ...
    if mode == 'train':
        ...
        rhand_position_net.apply(init_weights)
        rhand_rotation_net.apply(init_weights)
        lhand_position_net.apply(init_weights)
        lhand_rotation_net.apply(init_weights)

        hand_trans_net.apply(init_weights)
        hand_trans_backbone.init_weights()

    model = Model(body_backbone, body_box_net, hand_roi_net, rhand_position_net, rhand_rotation_net, lhand_position_net, lhand_rotation_net, hand_trans_net)
    return model
