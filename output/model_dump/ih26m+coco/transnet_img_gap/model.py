...
from nets.loss import BCELoss, CoordLoss, ParamLoss
...
class Model(nn.Module):
    def __init__(self, body_backbone, body_box_net, hand_roi_net, hand_position_net, hand_rotation_net, hand_trans_net):
        ...
        self.trainable_modules = [self.body_backbone, self.body_box_net, self.hand_roi_net, self.hand_position_net, self.hand_rotation_net, self.hand_trans_net, self.joint_regressor]
    
    def forward_rotation_net(self, hand_feat, hand_coord):
        root_pose_6d, pose_param_6d, shape_param, cam_param, contact_feat = self.hand_rotation_net(hand_feat, hand_coord)
        ...
        return root_pose, pose_param, shape_param, cam_trans, contact_feat
    ...
    def forward(self, inputs, targets, meta_info, mode):
        ...
        mano_root_pose, mano_hand_pose, mano_shape, cam_trans, contact_feat = self.forward_rotation_net(hand_feat, joint_img.detach())
        ...
        # joint contact features
        lcontact_feat = contact_feat[:lhand_num,:]
        rcontact_feat = contact_feat[lhand_num:,:]
        ...
        # relative translation and contact
        #rel_trans, joint_contact = self.hand_trans_net(rjoint_img.detach(), ljoint_img.detach(), rhand_bbox.detach(), lhand_bbox.detach(), rcontact_feat, lcontact_feat)
        rel_trans, joint_contact = self.hand_trans_net(rjoint_img.detach(), ljoint_img.detach(), rhand_bbox.detach(), lhand_bbox.detach(), rcontact_feat, lcontact_feat, inputs['img'])
        ...
        if mode == 'train':
            ...
            loss['joint_contact'] = self.bce_loss(joint_contact, targets['joint_contact'], meta_info['contact_valid'])
            ...
            out['joint_contact'] = joint_contact
