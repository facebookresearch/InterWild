...
class RotationNet(nn.Module):
    def __init__(self):
        ...
        self.joint_num = mano.sh_joint_num
        self.conv_contact = make_conv_layers([2048,512], kernel=1, stride=1, padding=0)
    ...
    def forward(self, hand_feat, joint_img):
        ...
        # contact
        contact_feat = self.conv_contact(hand_feat)
        contact_feat = sample_joint_features(contact_feat, joint_img[:,:,:2]) # batch_size, joint_num, feat_dim
        ...
        return root_pose, pose_param, shape_param, cam_param, contact_feat 
...
class TransNet(nn.Module):
    def __init__(self, backbone):
        ...
        #self.contact_out = nn.ModuleList([make_linear_layers([512+self.joint_num*2*3,1], relu_final=False) for i in range(self.joint_num*2)])
        self.contact_out = nn.ModuleList([make_linear_layers([512+self.joint_num*2,1], relu_final=False) for i in range(self.joint_num*2)])
        #self.contact_out = nn.ModuleList([make_linear_layers([512,1], relu_final=False) for i in range(self.joint_num*2)])
        #self.contact_out = nn.ModuleList([make_linear_layers([self.joint_num*2,1], relu_final=False) for i in range(self.joint_num*2)])
        self.conv = make_conv_layers([self.joint_num*2*cfg.input_hm_shape[0],64], kernel=3, stride=1, padding=1)
        self.fc = make_linear_layers([512,3], relu_final=False)
        #self.fc = make_linear_layers([512+self.joint_num*2,512,3], relu_final=False)
    ...
    def forward(self, rjoint_img, ljoint_img, rhand_bbox, lhand_bbox, rcontact_feat, lcontact_feat, img):
        ...
        # joint contact
        contact_feat = torch.cat((rcontact_feat, lcontact_feat),1)
        #joint_img = torch.cat((rjoint_img, ljoint_img),1)
        #joint_img = joint_img[:,None,:,:].repeat(1,self.joint_num*2,1,1).view(-1,self.joint_num*2,self.joint_num*2*3)
        #contact_feat = torch.cat((contact_feat, joint_img),2)
        rjoint_offset = ljoint_img[:,None,:,:2] - rjoint_img[:,:,None,:2]
        ljoint_offset = rjoint_img[:,None,:,:2] - ljoint_img[:,:,None,:2]
        joint_offset = torch.cat((rjoint_offset, ljoint_offset),1).view(-1,self.joint_num*2,self.joint_num*2)
        contact_feat = torch.cat((contact_feat, joint_offset),2)
        contact_feat = contact_feat.permute(0,2,1) # batch_size, feat_dim, joint_num
        joint_contact = []
        for j in range(self.joint_num*2):
            joint_contact.append(torch.sigmoid(self.contact_out[j](contact_feat[:,:,j])))
        joint_contact = torch.stack(joint_contact, 1)
        ...
        #hand_feat = self.backbone(self.conv(hand_hm), stage='late').mean((2,3))
        hand_feat = self.backbone(hand_img).mean((2,3))
        #joint_contact = torch.cat((rjoint_contact, ljoint_contact),1).view(-1,self.joint_num*2)
        #hand_feat = torch.cat((hand_feat, joint_contact),1)
        rel_trans = self.fc(hand_feat)
        return rel_trans, joint_contact
