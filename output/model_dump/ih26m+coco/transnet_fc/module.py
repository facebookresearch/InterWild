...
from nets.layer import make_conv_layers, make_deconv_layers, make_linear_layers, ResBlock1D
...
class TransNet(nn.Module):
    def __init__(self):
        ...
        self.joint_num = mano.sh_joint_num
        self.fc1 = make_linear_layers([2*self.joint_num*3,512], relu_final=True, use_bn=True)
        self.resblocks = nn.Sequential(*[ResBlock1D(512) for _ in range(4)])
        self.fc2 = make_linear_layers([512,3], relu_final=False, use_bn=False)
    ...
    def forward(self, rjoint_img, ljoint_img, rhand_bbox, lhand_bbox):
        ...
        # hand heatmap
        feat = torch.cat((rjoint_img, ljoint_img),1).view(-1,2*self.joint_num*3)
        feat = self.fc1(feat)
        feat = self.resblocks(feat)
        rel_trans = self.fc2(feat)
        return rel_trans
