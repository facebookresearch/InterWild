...
class TransNet(nn.Module):
    def __init__(self, backbone):
        ...
        self.conv = make_conv_layers([self.joint_num*2,64], kernel=3, stride=1, padding=1)
    ...
    def render_gaussian_heatmap(self, joint_coord):
        ...
        x = joint_coord[:,:,0,None,None]; y = joint_coord[:,:,1,None,None]; 
    ...
    def forward(self, rjoint_img, ljoint_img, rhand_bbox, lhand_bbox):
        ...
        rhand_hm = rhand_hm.view(-1,self.joint_num,cfg.input_hm_shape[1],cfg.input_hm_shape[2]) 
        lhand_hm = self.render_gaussian_heatmap(ljoint_img)
        lhand_hm = lhand_hm.view(-1,self.joint_num,cfg.input_hm_shape[1],cfg.input_hm_shape[2])
