...
class Model(nn.Module):
    ...
    def get_camera_trans_union(self, cam_param):
        ...
        t_xy = cam_param[:,:2]
        gamma = torch.sigmoid(cam_param[:,2]) # apply sigmoid to make it positive
        k_value = torch.FloatTensor([math.sqrt(cfg.focal[0]*cfg.focal[1]*cfg.camera_union_3d_size*cfg.camera_union_3d_size/(cfg.input_hm_shape[1]*cfg.input_hm_shape[2]))]).cuda().view(-1)
        t_z = k_value * gamma
        cam_trans = torch.cat((t_xy, t_z[:,None]),1)
        return cam_trans
    ...
    def get_coord(self, root_pose, hand_pose, shape, cam_trans, cam_trans_from_union, hand_type):
        ...
        x = (joint_cam[:,:,0].detach() + cam_trans_from_union[:,None,0]) / (joint_cam[:,:,2].detach() + cam_trans_from_union[:,None,2]) * cfg.focal[0] + cfg.princpt_union[0]
        y = (joint_cam[:,:,1].detach() + cam_trans_from_union[:,None,1]) / (joint_cam[:,:,2].detach() + cam_trans_from_union[:,None,2]) * cfg.focal[1] + cfg.princpt_union[1]
        x = x / cfg.input_hm_shape[2] * cfg.output_hand_hm_shape[2]
        y = y / cfg.input_hm_shape[1] * cfg.output_hand_hm_shape[1]
    ...
    def forward(self, inputs, targets, meta_info, mode):
        ...
        rcam_trans_from_union, rel_trans, hand_bbox_union = self.hand_trans_net(rjoint_img.detach(), ljoint_img.detach(), rhand_bbox.detach(), lhand_bbox.detach())
