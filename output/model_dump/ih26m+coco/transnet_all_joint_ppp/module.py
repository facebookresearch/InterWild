...
class TransNet(nn.Module):
    def __init__(self, backbone):
        ...
        self.fc = make_linear_layers([mano.th_joint_num*(512+3),3], relu_final=False)
    ...
    def forward(self, rjoint_img, ljoint_img, rhand_bbox, lhand_bbox):
        ...
        joint_img = torch.stack((joint_img[:,:,0] / 8, joint_img[:,:,1] / 8, joint_img[:,:,2] / 8),2)
        joint_feat = sample_joint_features(hand_feat, joint_img.detach())
        joint_feat = torch.cat((joint_feat, joint_img),2).view(-1,mano.th_joint_num*(512+3))
        rel_trans = self.fc(joint_feat)
        return rel_trans
