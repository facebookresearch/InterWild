...
from nets.layer import make_conv_layers, make_deconv_layers, make_linear_layers, TransformerEncoder
...
class TransNet(nn.Module):
    def __init__(self):
        ...
        self.joint_num = mano.sh_joint_num
        self.conv = make_conv_layers([3,128], kernel=1, stride=1, padding=0, bnrelu_final=False)
        self.joint_query = nn.Parameter(torch.zeros(self.joint_num*2, 128))
        self.transformer_encoder = nn.Sequential(*[TransformerEncoder(128,512,8) for _ in range(4)])
        self.layer_norm = nn.LayerNorm(128)
        self.fc = nn.Linear(2*128,3)
    ...
    def forward(self, rjoint_img, ljoint_img, rhand_bbox, lhand_bbox):
        ...
        rhand_bbox, lhand_bbox = rhand_bbox.clone(), lhand_bbox.clone()
        ...
        # hand heatmap
        feat = torch.cat((rjoint_img, ljoint_img),1).view(-1,3,1,1)
        feat = self.conv(feat).view(-1,self.joint_num*2,128).permute(1,0,2) + self.joint_query[:,None,:]
        feat = self.transformer_encoder(feat).permute(1,0,2)
        feat = torch.stack((feat[:,mano.th_root_joint_idx['right'],:], feat[:,mano.th_root_joint_idx['left'],:]),1)
        feat = self.layer_norm(feat).view(-1,2*128)
        rel_trans = self.fc(feat)
        return rel_trans
