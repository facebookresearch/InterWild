...
def download(capture_id):
    ...
    os.makedirs(osp.join(capture_id, 'keypoints_orig'), exist_ok=True)
    current_path = os.path.dirname(os.path.abspath(__file__))
    os.chdir(osp.join(current_path, capture_id, 'keypoints_orig'))

    # download
    for name in name_list[capture_id]:
        cmd = 'wget https://fb-baas-f32eacb9-8abb-11eb-b2b8-4857dd089e15.s3.amazonaws.com/ReInterHand/' + capture_id + '/keypoints_orig/keypoints_orig.tar.gz' + name
        os.system(cmd)

    # decompress
    cmd = 'cat keypoints_orig.tar.gz* | tar zxvf -'
    os.system(cmd)
